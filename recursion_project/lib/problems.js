...
function sumArray(array, sum = 0) {
    ...
    const sliceArray = array.slice(0, array.length-1);
    return sumArray(sliceArray, sum);
}
// time o(n)
...
function reverseString(str, res = '') {
    ...
    sliced = str.slice(0,str.length - 1);
    return reverseString(sliced, res);
}
// time o(n) space (n)
    ...
    switch(exponent){
        ...
        default:
            ...
            if(exponent % 2 === 0){
                ...
                half = pow(base, Math.floor(exponent / 2))
...
// Write a function, fileFinder(directories, targetFile), 
// that accepts an object representing directories and a string respresenting a filename.
...
function fileFinder(directories, targetFile) {
    for (let el in directories) {
        if (el === targetFile || fileFinder(directories[el], targetFile) ) return true;
    };
    return false;
}
...
function pathFinder(directories, targetFile){
    ...
    for (let el in directories) {
        if (el === targetFile) return '/' + el;
        let path = pathFinder(directories[el], targetFile)
        if (path) return el + path;  
    };
    return null;
}
